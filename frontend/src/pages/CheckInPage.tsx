...
import { CheckCircle2, AlertCircle, MapPin, Calendar, ArrowLeft, Sparkles } from 'lucide-react';
...
export function CheckInPage() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[80vh] px-4 py-8">
      ...
      >
        <Link to="/" className="flex items-center gap-2 text-sm text-muted-foreground hover:text-primary mb-6 transition-colors group">
        ...
        <Card className="border-2">
            ...
            {!reservation ? (
              <div className="bg-destructive/10 border border-destructive/20 p-4 rounded-lg flex gap-3 items-start">
                ...
                <div className="text-sm">
                  <p className="font-semibold text-destructive">Aucune réservation trouvée</p>
                  <p className="text-destructive/80 mt-1">
                    Vous n'avez pas de réservation active pour la place {spotId} aujourd'hui.
                  </p>
                </div>
              </div>
              ...
              >
                ...
                <div className="space-y-1">
                  <p className="text-xl font-bold text-emerald-700 dark:text-emerald-300">Check-in réussi !</p>
              ...
              <div className="space-y-4">
                <div className="p-4 rounded-lg border bg-muted/50">
                  <div className="flex justify-between items-center mb-4">
                    <span className="text-sm font-medium">Détails de la réservation</span>
                    <Badge>Active</Badge>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-center gap-2 text-sm">
                      <Calendar className="h-4 w-4 text-muted-foreground" />
                      <span>{format(reservation.from, "EEEE dd MMMM", { locale: fr })}</span>
                    </div>
                    <div className="flex items-center gap-2 text-sm">
                      <MapPin className="h-4 w-4 text-muted-foreground" />
                      <span>Place {reservation.spotId} - Rangée {reservation.spotId[0]}</span>
                    </div>
                  </div>
                </div>
          ...
          <CardFooter className="flex flex-col gap-4 border-t pt-6">
            ...
            <p className="text-[10px] text-center text-muted-foreground">
              Le check-in doit être effectué avant 11h00. Passé ce délai, la place est remise en disponibilité pour la journée.
            </p>
          </CardFooter>
        </Card>
      </motion.div>
